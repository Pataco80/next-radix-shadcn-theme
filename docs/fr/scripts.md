...
- `translate` : Traduit des fichiers Markdown spécifiques en français

  ```bash
  pnpm translate <fichier1.md> <fichier2.md> ...
  ```

- `translate:all` : Traduit tous les fichiers Markdown prédéfinis en français

  ```bash
  pnpm translate:all
  ```

- `translate:find` : Recherche et traduit tous les fichiers Markdown du projet en français
  ```bash
  pnpm translate:find
  ```

Les fichiers traduits sont sauvegardés dans le dossier `.private/translations/fr/`.
