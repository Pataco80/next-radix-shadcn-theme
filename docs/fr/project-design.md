...
### UtilisationCes variables d'espacement peuvent être utilisées pour les marges, les rembourrages, les écarts de grille et d'autres propriétés spatiales :
...
Le texte de base du site utilise `--step-0` avec une hauteur de ligne de `--lh-0`.
...
Le texte de petite taille (comme les notes de bas de page, les légendes) utilise `--step--1` avec une hauteur de ligne de `--lh--1`.
