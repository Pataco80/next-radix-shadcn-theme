...
/**
 * Script pour traduire des fichiers Markdown (.md) en français
 * Usage: ts-node scripts/translate-fr.ts <fichier1.md> <fichier2.md> ...
 * Usage: ts-node scripts/translate-fr.ts --all (pour traduire tous les fichiers .md prédéfinis)
 * Usage: ts-node scripts/translate-fr.ts --find (pour trouver et traduire tous les fichiers .md du projet)
 * Exemple: ts-node scripts/translate-fr.ts README.md CONTRIBUTING.md
 */
...
async function translateFileList(files: string[]): Promise<void> {
	try {
		...
		for (const file of markdownFiles) {
			...
			console.log(`Traduction de ${file}...`)
